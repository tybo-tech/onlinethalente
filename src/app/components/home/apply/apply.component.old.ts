...
export class ApplyComponent implements OnInit, OnDestroy {
  ...
  // Application flow state (legacy - now managed by service)
  totalSteps = 6;
  stepTitles = [
  ...
  private loadApplicationData(application: Application): void {
    // Map application data to form data structure
    this.applicationData = {
      ...
      // Employment info
      employmentStatus: application.employmentInfo?.employmentStatus || 'employed',
      employerName: application.employmentInfo?.employerName || '',
      jobTitle: application.employmentInfo?.jobTitle || '',
      monthlyIncome: application.employmentInfo?.monthlyIncome || 0,
      additionalIncome: application.employmentInfo?.additionalIncome || 0,
      employmentStartDate: application.employmentInfo?.employmentStartDate || '',
      ...
      accountType: application.bankDetails?.accountType || 'savings',
    ...
    this.initialData = { ...this.applicationData };
  }
  ...
  private saveCurrentStepData(): void {
    // Update the application service with current form data
  ...
  private buildUpdateObject(): Partial<Application> {
  ...
  onSave(data: any) {
    ...
    if (this.currentStep < this.totalSteps) {
      // If not on last step, save data and move to next step
      this.saveCurrentStepData();
    ...
    this.applicationService.submitApplication().then(success => {
      if (success) {
  ...
  onCancel() {
    // Save current progress via application service
    this.saveCurrentStepData();
